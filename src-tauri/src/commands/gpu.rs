...
pub fn get_gpu_stats() -> GpuStats {
    ...
    // nvml_err is used further down in WMI fallback error messages.
    let nvml_err: Option<String>;
    let nvml_gpus: Vec<GpuInfo> = {
        #[cfg(feature = "nvidia")]
        {
            // Initialise NVML exactly once and reuse the handle.
            match nvml_wrapper::Nvml::init() {
                Err(_) => {
                    nvml_err = Some("NVML not available (NVIDIA driver not installed?)".to_string());
                    Vec::new()
                }
                Ok(nvml) => {
                    nvml_err = None;
                    ...
                    if let Ok(count) = nvml.device_count() {
                        for i in 0..count {
                            if let Ok(device) = nvml.device_by_index(i) {
                                ...
                                let gpu_usage = device.utilization_rates().map(|u| u.gpu as f32).unwrap_or(0.0);
                                let temperature = device.temperature(nvml_wrapper::enum_wrappers::device::TemperatureSensor::Gpu).ok().map(|t| t as f32);
                                let power_watts = device.power_usage().ok().map(|p| p as f32 / 1000.0);
                                let power_limit_watts = device.power_management_limit().ok().map(|p| p as f32 / 1000.0);
                                ...
                                gpus.push(GpuInfo {
                                    ...
                                    fan_speed_percent,
                                    fan_rpm: None,
                                    card_path: uuid.unwrap_or_default(),
                                    encoder_percent,
                                    decoder_percent,
                                });
                            }
        ...
        {
            nvml_err = Some("NVIDIA support not compiled in (enable 'nvidia' feature)".to_string());
            Vec::new()
        }
    };
